...
This vignette describes the UMPFACK interace implemeneted in
`umfpackr` in detail.
...
On Windows SuiteSparse is it difficult to install SuiteSparse and use
it in R.
Fortunately the SuiteSparse libraries UMFPACK and AMD can compiled
with GNU Fortran. These libraries are sufficient to 
solve sparse linear
...
In R package `umfpackr` following ordering methods can be chosen:
...
More details can be found in the [_UMFPACK User Guide_](UMFPACK_UserGuide.PDF) 
...
This argument should be a named list. To select the METIS
